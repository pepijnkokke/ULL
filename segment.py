...
def find_enclosing_boundaries(boundaries, i):
    ...
    upper = i + 1
    while upper < len(boundaries) and boundaries[upper] != 1:
        upper += 1

    return lower, upper


def gibbs_iteration(corpus, boundaries, n, word_counts, i, rho=2.0, alpha=0.5):
    # utterance boundaries are unambiguous
    if corpus.utt_boundaries[i] == 1:
        return 0

    phoneme = corpus.text[i]
...
def main():
    ...
    with Parallel(n_jobs=cpu_count()) as pool:
