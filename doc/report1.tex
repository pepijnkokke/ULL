...
% - results
% - conclusion
In this report, we apply word segmentation on simple utterances spoken to infants. We use a generative model to find the most likely placement of word boundaries and infer with Gibbs Sampling. Since the model assumes words to be independent, it has a tendency to undersegment. We find the performance to be invariant under various parameter initialisations.
\end{abstract}
...
\item $p_\# = \{0.2, 0.5, 0.8\}$
\end{itemize}
Afterwards, we computed precision, recall and f zero for the words found, the lexical types found and the placement of word boundaries.
